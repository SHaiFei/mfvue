<template>
    <div class="all-data-detail">
        <el-row class="elRow">
            <div class="elRowLeft">
                <span class="font-12 spanText">检查内容</span>
        ...
        <el-table :data="tableData" stripe style="width:100%;margin-top: 10px" @selection-change='tableDataSelect'>
            ...
            <el-table-column prop="type" label="检查依据" align="center">
        ...
        <el-dialog :title="title" :visible.sync="dialogFormVisible" v-if="dialogFormVisible" width="40%" :close-on-click-modal="false">
            <specialForm v-if="status == 'add'" @closeDialog="closeDialog"></specialForm>
            <specialForm v-else :rowData="rowData" @closeDialog="closeDialog"></specialForm>
        </el-dialog>
    </div>
</template>

<script>
import { specialList, securityDel } from '@/api/engineering/system/library'
...
export default {
    ...
    methods: {
        getTableDataList () { // 题库维护列表
            specialList(this.queryInfo).then(res => {
                if (res.code == 200) {
