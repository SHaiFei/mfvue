<template>
    <div>
        <el-row :gutter="24">
            <el-col :span="12">
                <div>待检工程</div>
                <el-table :data="tableData" stripe style="width:100%;margin-top: 10px">
                    <el-table-column type="index" label="序号" width="50px" align="center"></el-table-column>
                    <el-table-column prop="underCode" label="工程编号" align="center" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="underName" label="工程名称" align="center" show-overflow-tooltip></el-table-column>
                </el-table>
            </el-col>
            <el-col :span="12">
                <div>执法人员</div>
                <el-table :data="tableData2" stripe style="width:100%;margin-top: 10px">
                    <el-table-column type="index" label="序号" width="50px" align="center"></el-table-column>
                    <el-table-column prop="name" label="名字" align="center" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="duty" label="职务" align="center" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="phone" label="手机号" align="center" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="zfzh" label="执法证号" align="center" show-overflow-tooltip></el-table-column>
                </el-table>
            </el-col>
        </el-row>

        <div slot="footer" class="dialog-footer" style="text-align: right">
            <el-button @click="close">取 消</el-button>
        </div>
    </div>
</template>

<script>
import { enforcementTableList, enginTableList } from "@/api/engineering/spotcheck"

export default {
    props: ['ssjrwId'],
    data () {
        return {
            tableData: [],
            tableData2: [],

        }
    },
    created () {
        let params = {
            ssjrwId: this.ssjrwId
        }
        this.getEnforcement(params)
        this.getEngin(params)
    },
    methods: {
        getEnforcement (params) {
            enforcementTableList(params).then(res => {
                if (res.code == 200) {
                    this.tableData2 = res.rows
                }
            })
        },
        getEngin (params) {
            enginTableList(params).then(res => {
                if (res.code == 200) {
                    this.tableData = res.rows
                }
            })
        },
        close () {
            this.$emit("closeDialog")
        }
    },
}
</script>


<style lang='scss' scoped>
.el-date-editor {
    width: 100%;
}
.dialog-footer {
    margin-top: 20px;
}
</style>
