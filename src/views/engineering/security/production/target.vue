    ...
    <div class="all-data-detail">
        <el-row class="elRow">
            <div class="elRowLeft">
                ...
                <span class="font-12 spanText">责任书名称</span>
                <el-input v-model="queryInfo.dutyName" clearable class="elinput200"></el-input>
                <span class="font-12 spanText">签字日期</span>
                <el-input v-model="queryInfo.signatureDate" clearable class="elinput200"></el-input>
        ...
        <el-table ref="multipleTable" :data="tableData" stripe style="width:100%;margin-top: 10px" @selection-change='tableDataSelect'>
            ...
            <el-table-column prop="dutyName" label="责任书名称" align="center"></el-table-column>
            <el-table-column prop="signatory" label="上级主管部门" align="center"></el-table-column>
            <el-table-column prop="upstream" label="签字人" align="center"></el-table-column>
            <el-table-column prop="signatureDate" label="签字日期" align="center"></el-table-column>
            <el-table-column prop="synopsis" label="内容摘要" align="center"></el-table-column>
        </el-table>
        ...
        <el-dialog :title="title" :visible.sync="dialogFormVisible" v-if="dialogFormVisible" width="40%" :close-on-click-modal="false" class="dialogForm">
            <targetForm v-if="status == 'add'" @closeDialogFormVisible="closeDialog"></targetForm>
            <targetForm v-else :rowData="rowData" @closeDialogFormVisible="closeDialog"></targetForm>
        </el-dialog>
    </div>
</template>
...
import { tableDataList, targetDel } from '@/api/engineering/security/production'
        ...
        del () { // 题库列表删除
            if (this.rowData.length != 1) {
                ...
                this.msgBox('确定删除?').then(res => {
                    if (res == 'confirm') {
                        targetDel(this.rowData[0].goaId).then(res => {
                            if (res.code == 200) {
