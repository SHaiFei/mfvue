<template>
    <div class="all-data-detail">
        <el-row class="elRow">
            <div class="elRowLeft">
                <span class="font-12 spanText">禁用用途名称</span>
...
<script>
import { disableMaintenanceList } from '@/api/engineering/security/usePlanning'
export default {
    data () {
        return {
            queryInfo: { // 分页
    ...
    methods: {
        getTableDataList () { // 题库维护列表
            disableMaintenanceList(this.queryInfo).then(res => {
                if (res.code == 200) {
